...
describe Carnival do
  ...
  describe '#draw_lottery_winner' do
    ...
    it 'returns nil if there are no contestants' do
      ...
      expect(jeffco_fair.draw_lottery_winner(ferris_wheel)).to be_nil
    end
  end

  describe '#announce_lottery_winner' do
    it 'returns an announcement of the winner for a specific ride' do
      jeffco_fair = Carnival.new("Jefferson County Fair")
      ferris_wheel = Ride.new({name: 'Ferris Wheel', cost: 0})
      bumper_cars = Ride.new({name: 'Bumper Cars', cost: 10})
      scrambler = Ride.new({name: 'Scrambler', cost: 15})
    ...
    it 'returns no winners annoucement if no contestants available' do
      jeffco_fair = Carnival.new("Jefferson County Fair")
      ferris_wheel = Ride.new({name: 'Ferris Wheel', cost: 0})
      bumper_cars = Ride.new({name: 'Bumper Cars', cost: 10})
      scrambler = Ride.new({name: 'Scrambler', cost: 15})
      ...
      expect(jeffco_fair.announce_lottery_winner(ferris_wheel)).to eq expected
    end
  end
end
